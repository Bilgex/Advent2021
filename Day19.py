    ...
    for point1 in keys1:
        for point2 in keys2.copy():
            ...
            for key in dis_dict[scanner1][point1]&dis_dict[scanner2][point2]:
                countin+=dis_dict[scanner1][point1][key]+dis_dict[scanner2][point2][key]
            if countin>=12:
                common+=1
...
def composer_old(map2,map1):
    f_map=[]
    for pointer in map1:
        f_map.append(abs(map2[abs(pointer)-1])*np.sign(pointer*map2[abs(pointer)-1]))
    return f_map
def apply(map,point):
    range_point=[0,0,0]
...
def decoder(scanner1,scanner2,carry_over=[]):
    ...
    translated_beacons=[np.array(apply(go_back,[point[0],point[1],list(point)[2]*(-1)])) if oriented else np.array(apply(go_back,point))  for point in matchings2]
    ...
    orient_rotate=[np.array(apply(go_back,[point[0],point[1],list(point)[2]*(-1)])) if oriented else np.array(apply(go_back,point))  for point in scan_dict[scanner2]+carry_over]
...
def solve_all(path,i=0):
    if i==len(path)-1:
        final=carry_overs[0]
        [final.append(x) for x in scan_dict[0] if not np.any([np.all(np.equal(x,y)) for y in carry_overs[0]])]
        ...
        for node in path[i]:
            ...
            trans_vec,go_back,oriented,translated=decoder(where_to,node,carry_overs[node])
            translation_vecs[(where_to,node)]=trans_vec
            heyallam=[];[heyallam.append(x) for x in translated if not np.any([np.all(np.equal(x,y)) for y in heyallam])]
            carry_overs[where_to]=carry_overs[where_to]+translated
            new_locations=[]
            for scan_node in carry_overs_scanners[node]["Scanner locations"]:
                new_locations.append(np.array(apply(go_back,[scan_node[0],scan_node[1],list(scan_node)[2]*(-1)]))-trans_vec if oriented else np.array(apply(go_back,scan_node))-trans_vec)
            new_locations.append((-1)*trans_vec)
            carry_overs_scanners[where_to]["Scanner numbers"]=carry_overs_scanners[where_to]["Scanner numbers"]+carry_overs_scanners[node]["Scanner numbers"]+[node]
            carry_overs_scanners[where_to]["Scanner locations"]=carry_overs_scanners[where_to]["Scanner locations"]+new_locations
...
scanner_loc=sorted([(0,np.array([0,0,0]))]+list(zip(carry_overs_scanners[0]["Scanner numbers"],carry_overs_scanners[0]["Scanner locations"])))


manhattan_distances=[]
for scanner1 in range(len(scanner_loc)):
    for scanner2 in range(scanner1+1,len(scanner_loc)):
        manhattan_distances.append(((scanner1,scanner2),np.sum(np.absolute(scanner_loc[scanner1][1]-scanner_loc[scanner2][1]))))
