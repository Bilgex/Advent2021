...
class Operator(Packet):
    ...
    def sum_versions(self):
        ...
        for subpacket in self.subpackets:           
            sum+=subpacket.sum_versions() if subpacket.type_id!=4 else subpacket.version 
        return sum
    def get_value(self):
        values =[sub.get_value() for sub in self.subpackets]     
        if self.type_id==0:  return sum(values)
        elif self.type_id==1:return prod(values)          
        elif self.type_id==2:return min(values)
        elif self.type_id==3:return max(values)
        elif self.type_id==5:return 1 if values[0]>values[1] else 0
        elif self.type_id==6:return 1 if values[0]<values[1] else 0
        else:                return 1 if values[0]==values[1] else 0

                


pack=Packet(binary_code)
answer=pack.get_value()
print(answer)
